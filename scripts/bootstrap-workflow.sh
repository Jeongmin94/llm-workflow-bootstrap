...
Options:
  ...
  --with-notion-mcp   Install Notion MCP docs and example contract
...
if [[ -z "$TARGET" || -z "$PROJECT_NAME" ]]; then
  echo "Error: --target and --project-name are required." >&2
  usage
  exit 1
fi
...
for raw in "${MODEL_ITEMS[@]}"; do
  ...
  case "$model" in
    codex|claude|gemini|antigravity)
      ...
      if [[ "$model" == "antigravity" ]]; then
        ANTIGRAVITY_ALIAS_USED=1
      fi
      if [[ " ${MODELS[*]-} " != *" $canonical_model "* ]]; then
    ...
    "")
      ;;
    *)
      echo "Error: unsupported model '$model'. Supported: codex, claude, gemini (or antigravity alias)" >&2
      exit 1
      ;;
  esac
done
...
- approval-policy: never'
    ...
    claude)
      ...
      ;;
    gemini)
      printf '%s' '- run with Gemini CLI/Antigravity equivalent unrestricted filesystem/command/network permissions'
      ;;
    *)
...
title_for_model() {
  ...
  case "$model" in
    ...
    gemini) printf '%s' 'Gemini (Antigravity)' ;;
    *) printf '%s' "$model" ;;
  esac
}
...
if [[ $ANTIGRAVITY_ALIAS_USED -eq 1 ]]; then
  echo "Note: 'antigravity' was treated as alias for 'gemini' and generated under .agent/skills."
fi
if [[ $WITH_NOTION_MCP -eq 1 ]]; then
